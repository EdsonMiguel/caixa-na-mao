...
export function GerenciarPagamentos({
  ...
  const [metodoPagamento, setMetodoPagamento] = useState<
    'dinheiro' | 'pix' | 'cartao-debito' | 'cartao-credito'
  >('dinheiro');
          ...
          {Object.keys(grupos).length === 0 ? (
            ...
            <div className="space-y-4">
              {Object.entries(grupos).map(([clienteKey, pedidosDoCliente]) => {
                const totalCliente = pedidosDoCliente.reduce(
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 mb-3">
                          ...
                          {pedidosDoCliente.length === 1 ? 'pedido' : 'pedidos'} •
                          {pedidosDoCliente.reduce(
                            (total, p) =>
                              total + p.itens.reduce((sum, item) => sum + item.quantidade, 0),
                            0,
                          )}{' '}
                          itens
                        </p>
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-gray-200">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {pedidosDoCliente.length}{' '}
                          {pedidosDoCliente.length === 1 ? 'pedido' : 'pedidos'} para pagamento
                        </p>
                      </div>
                      <button
                        onClick={() =>
                          abrirModalPagamento(
                            clienteKey === 'sem-cliente' ? null : clienteKey,
                            pedidosDoCliente,
                          )
                        }
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600">
                          {pagamento.pedidosIds.length}{' '}
                          {pagamento.pedidosIds.length === 1 ? 'pedido' : 'pedidos'} •
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {pedidosEntregues
                .filter((p) =>
                  clienteSelecionado ? p.clienteId === clienteSelecionado : !p.clienteId,
                )
                .map((pedido) => (
          ...
          {pedidosSelecionados.length > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-600 mt-1">
                ...
                {pedidosSelecionados.length === 1 ? 'pedido selecionado' : 'pedidos selecionados'}
              </p>
            </div>
          )}
